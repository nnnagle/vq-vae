...
# ---------------------------------------------------------------------------
# Type-stratified gamma diagnostics
# ---------------------------------------------------------------------------
...
def _kmeans_torch(
    ...
    """Lloyd k-means on a CPU float64 tensor.
    ...
    for _ in range(n_iter):
        ...
        for k in range(K):
            m = labels == k
            new_centers[k] = X[m].mean(dim=0) if m.any() else centers[k]
...
def compute_type_stratified_diagnostics(
    loader: DataLoader,
    feature_builder: FeatureBuilder,
    model: RepresentationModel,
    device: torch.device,
    ...
    """Cluster z_type embeddings and report per-cluster FiLM gamma and z_phase
    temporal variance statistics.

    Tests whether the model gates the phase pathway selectively by type:
    a few clusters with gamma >> 1 (strong phase signal) alongside many near
    gamma ≈ 0 (suppressed phase) would confirm the expected structure.

    Args:
        n_clusters:  Number of k-means clusters for z_type.
        max_pixels:  Cap on pixels collected for clustering (subsampled uniformly).

    Returns:
        dict with ``cluster_stats`` (list of per-cluster dicts, sorted by
        descending gamma magnitude), ``gamma_mag_percentiles`` (dict of
        pixel-level percentiles), and metadata.
    """
    model.eval()
    d_type = model.z_type_dim

    ztype_list: List[torch.Tensor] = []
    gamma_mag_list: List[torch.Tensor] = []   # mean |gamma| per pixel (scalar)
    zphase_tvar_list: List[torch.Tensor] = []  # within-pixel temporal var of z_phase (scalar)
    n_collected = 0
    ...
    with torch.no_grad():
        for batch in _iter_batches(loader, max_batches):
            if n_collected >= max_pixels:
                break
            Ximg, Xphase, _Yimg, M = extract_phase_batch_tensors(
                batch, feature_builder, device,
            )
            Bsz, _, H, W_sp = Ximg.shape
            ...
            z_type_det = z_type.detach()
            z_phase = model.forward_phase(Xphase, z_type_det)
            gamma, _beta = model.phase_film(z_type_det)

            halo_m = _halo_mask(H, W_sp, halo, device)
            full_mask = M & halo_m.unsqueeze(0)

            zt_pix = z_type_det.permute(0, 2, 3, 1)[full_mask]    # [N, d_type]
            gm_pix = gamma.permute(0, 2, 3, 1)[full_mask]          # [N, d_phase]
            # within-pixel temporal variance of z_phase, averaged over channels
            zp_pix = z_phase.permute(0, 3, 4, 1, 2)[full_mask]     # [N, d_phase, T]
            zp_tvar = zp_pix.var(dim=2, correction=0).mean(dim=1)   # [N]

            N_valid = zt_pix.shape[0]
            if N_valid == 0:
                continue

            # Subsample to stay under max_pixels
            remaining = max_pixels - n_collected
            if N_valid > remaining:
                idx = torch.randperm(N_valid, device=device)[:remaining]
                zt_pix = zt_pix[idx]
                gm_pix = gm_pix[idx]
                zp_tvar = zp_tvar[idx]

            ztype_list.append(zt_pix.cpu().float())
            gamma_mag_list.append(gm_pix.abs().mean(dim=1).cpu().float())
            zphase_tvar_list.append(zp_tvar.cpu().float())
            n_collected += zt_pix.shape[0]

    if n_collected == 0:
        raise RuntimeError("No valid pixels for type-stratified diagnostics.")

    Z = torch.cat(ztype_list, dim=0)                # [N, d_type]
    gamma_mag = torch.cat(gamma_mag_list, dim=0)    # [N]
    zp_tvar = torch.cat(zphase_tvar_list, dim=0)    # [N]

    logger.info(
        f"Type-stratified diagnostics: clustering {n_collected:,} pixels "
        f"into {n_clusters} clusters (d_type={d_type}) …"
    )
    labels, _centers = _kmeans_torch(Z.double(), n_clusters)
    ...
    for k in range(n_clusters):
        ...
        cluster_stats.append({
            "cluster": k,
            "count": count,
            "frac": count / n_collected,
            "gamma_mag_median": float(gm_k.median()),
            "gamma_mag_mean": float(gm_k.mean()),
            "gamma_mag_std": float(gm_k.std()),
            "zphase_tvar_mean": float(tv_k.mean()),
            "zphase_tvar_median": float(tv_k.median()),
        })

    cluster_stats.sort(key=lambda x: -x["gamma_mag_median"])

    percentile_levels = [1, 5, 10, 25, 50, 75, 90, 95, 99]
    gamma_mag_percentiles = {
        p: float(torch.quantile(gamma_mag, p / 100.0))
        for p in percentile_levels
    }

    return {
        "cluster_stats": cluster_stats,
        "n_pixels": n_collected,
        "n_clusters": n_clusters,
        "gamma_mag_percentiles": gamma_mag_percentiles,
    }


def log_type_stratified_diagnostics(diag: dict):
    """Log per-type-cluster gamma and z_phase temporal statistics."""
    n = diag["n_pixels"]
    K = diag["n_clusters"]
    logger.info(f"  [Type-stratified diagnostics] ({n:,} pixels, {K} clusters)")

    pct = diag["gamma_mag_percentiles"]
    logger.info("  Pixel-level gamma magnitude (mean |gamma|) distribution:")
    logger.info(
        f"    p1={pct[1]:.3f}  p5={pct[5]:.3f}  p10={pct[10]:.3f}  "
        f"p25={pct[25]:.3f}  p50={pct[50]:.3f}  p75={pct[75]:.3f}  "
        f"p90={pct[90]:.3f}  p95={pct[95]:.3f}  p99={pct[99]:.3f}"
    )

    logger.info("  Per-cluster stats (sorted by gamma magnitude, descending):")
    logger.info(
        f"  {'Cluster':>7} {'Pixels%':>8} {'γ_med':>7} "
        f"{'γ_mean':>7} {'γ_std':>6} {'z_phase_tvar':>13}"
    )
    logger.info(f"  {'-' * 58}")
    for s in diag["cluster_stats"]:
        logger.info(
            f"  {s['cluster']:>7} {s['frac']:>8.1%} "
            f"{s['gamma_mag_median']:>7.3f} {s['gamma_mag_mean']:>7.3f} "
            f"{s['gamma_mag_std']:>6.3f} {s['zphase_tvar_mean']:>13.6f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--n-type-clusters", type=int, default=20,
        help="K-means clusters for type-stratified gamma diagnostics (default: 20, 0 = skip)",
    )
    ...
    # Type-stratified gamma diagnostics
    if args.n_type_clusters > 0:
        logger.info("Computing type-stratified diagnostics on validation set...")
        type_diag = compute_type_stratified_diagnostics(
            val_loader, feature_builder, model, device,
            halo=args.halo,
            max_batches=args.max_batches_eval,
            n_clusters=args.n_type_clusters,
        )
        log_type_stratified_diagnostics(type_diag)
