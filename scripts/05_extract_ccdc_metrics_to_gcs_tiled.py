...
# GCS export configuration
GCS_BUCKET = "va_rasters"
GCS_DIR = "ccdc_summaries_tiled_2"
BASE_NAME_PREFIX = "ccdc_metrics_1986_2024"
...
class ArrayOps:
    ...
    def select_coefs_at_t(coefs_2d: ee.Image, mask_1d: ee.Image) -> ee.Image:
        ...
        Uses thecat/mask/reduce pattern that's proven to work in EE.
        """
        # Convert mask to float for arithmetic
        mask_float = mask_1d.toFloat()
    
        # We'll process each coefficient separately (unavoidable in EE)
        # But we'll do it efficiently using arraySlice
    
        # Strategy: For each coefficient column, multiply by mask and sum
        # Build a list of results and stack them
        
        def get_coef_value(coef_idx):
            """Extract coefficient at index, mask it, sum to get single value."""
            coef_idx = ee.Number(coef_idx)
            
            # Extract this coefficient column: [segment, 1]
            coef_col = coefs_2d.arraySlice(1, coef_idx, coef_idx.add(1))
            
            # Project to [segment]
            coef_1d = coef_col.arrayProject([0])
            
            # Multiply by mask (element-wise on [segment])
            masked = coef_1d.multiply(mask_float)
            
            # Sum to get the matching segment's value (or 0 if no match)
            value = masked.arrayReduce(ee.Reducer.sum(), [0]).arrayGet([0])
            
            return value
        
        # Process all 8 coefficients
        coef_indices = ee.List([0, 1, 2, 3, 4, 5, 6, 7])
        coef_images = coef_indices.map(get_coef_value)
        
        # Convert list to array image
        result = ee.ImageCollection.fromImages(coef_images).toBands().toArray()
        
        # Handle no-match case: if all zeros, replace with MISSING
        has_match = ArrayOps.has_any_segment(mask_1d.gt(0.5))
        missing_array = ee.Image(ee.Array([MISSING, MISSING, MISSING, MISSING, MISSING, MISSING, MISSING, MISSING])).toArray(0)
